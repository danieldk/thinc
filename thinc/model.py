...
class BaseModel(ABC, Generic[InT, OutT]):
    """Thinc models and layers interface."""
    ...
    @abstractmethod
    def copy(self: "BaseModel") -> "BaseModel":
        """
    ...
    def initialize(
        self, X: Optional[InT] = None, Y: Optional[OutT] = None
    ) -> "BaseModel":
    ...
    def remove_node(self, node: "BaseModel") -> None:
    ...
    def replace_node(self, old: "BaseModel", new: "BaseModel") -> bool:
        ...
        for node in list(self.walk(order="dfs_post")):
            if node is old:
                ...
                node.layers[:] = [
                    new if layer is old else layer for layer in node.layers
                ]
                for name in node.ref_names:
    ...
    def walk(self, *, order: str = "bfs") -> Iterable["BaseModel"]:
    ...
    def _walk_bfs(self) -> Iterable["BaseModel"]:
    ...
    def _walk_dfs(self, post_order: bool = False) -> Iterable["BaseModel"]:
        """Iterate out layers of the model, depth-first."""
        seen: Dict[int, Iterator["BaseModel"]] = dict()
        stack = [self]
    ...
    def __mul__(self, other: Any) -> "BaseModel":
        """Apply the function bound to the '*' operator."""
        if "*" not in self._context_operators.get():
            raise TypeError("Undefined operator: *")
        return self._context_operators.get()["*"](self, other)

    def __matmul__(self, other: Any) -> "BaseModel":
        """Apply the function bound to the '@' operator."""
        if "@" not in self._context_operators.get():
            raise TypeError("Undefined operator: @")
        return self._context_operators.get()["@"](self, other)

    def __div__(self, other: Any) -> "BaseModel":  # pragma: no cover
        """Apply the function bound to the '/' operator."""
        if "/" not in self._context_operators.get():
            raise TypeError("Undefined operator: /")
        return self._context_operators.get()["/"](self, other)

    def __truediv__(self, other: Any) -> "BaseModel":
        """Apply the function bound to the '/' operator."""
        if "/" not in self._context_operators.get():
            raise TypeError("Undefined operator: /")
        return self._context_operators.get()["/"](self, other)

    def __floordiv__(self, other: Any) -> "BaseModel":
        """Apply the function bound to the '//' operator."""
        if "//" not in self._context_operators.get():
            raise TypeError("Undefined operator: //")
        return self._context_operators.get()["//"](self, other)

    def __mod__(self, other: Any) -> "BaseModel":
        """Apply the function bound to the '%' operator."""
        if "%" not in self._context_operators.get():
            raise TypeError("Undefined operator: %")
        return self._context_operators.get()["%"](self, other)

    def __pow__(self, other: Any, **kwargs) -> "BaseModel":
        """Apply the function bound to the '**' operator."""
        if "**" not in self._context_operators.get():
            raise TypeError("Undefined operator: **")
        return self._context_operators.get()["**"](self, other)

    def __lshift__(self, other: Any) -> "BaseModel":
        """Apply the function bound to the '<<' operator."""
        if "<<" not in self._context_operators.get():
            raise TypeError("Undefined operator: <<")
        return self._context_operators.get()["<<"](self, other)

    def __rshift__(self, other: Any) -> "BaseModel":
        """Apply the function bound to the '>>' operator."""
        if ">>" not in self._context_operators.get():
            raise TypeError("Undefined operator: >>")
        return self._context_operators.get()[">>"](self, other)
    ...
    def __xor__(self, other: Any) -> "BaseModel":
        """Apply the function bound to the '^' operator."""
        if "^" not in self._context_operators.get():
            raise TypeError("Undefined operator: ^")
        return self._context_operators.get()["^"](self, other)
...
class Model(BaseModel, Generic[InT, OutT]):
    ...
    def __init__(
        ...
        *,
        ...
        layers: Sequence["BaseModel"] = [],
        shims: List[Shim] = [],
        attrs: Dict[str, Any] = {},
        refs: Dict[str, Optional["BaseModel"]] = {},
        ops: Optional[Union[NumpyOps, CupyOps]] = None,
    ):
        """Initialize a new model."""
        super().__init__(name, forward, init=init, ops=ops)
    ...
    def from_bytes(self, bytes_data: bytes) -> "BaseModel":
    ...
    def from_disk(self, path: Union[Path, str]) -> "BaseModel":
    ...
    def from_dict(self, msg: Dict) -> "BaseModel":
...
class DelegateModel(BaseModel, Generic[InT, OutT]):
    _layers: List["BaseModel"]
    ...
    def attrs(self) -> Dict[str, Any]:
        ...
        not reassign it.
        """
        return self.layers[0].attrs

    @property
    def param_names(self) -> Tuple[str, ...]:
        return self.layers[0].param_names

    @property
    def grad_names(self) -> Tuple[str, ...]:
        """Get the names of parameters with registered gradients (including unset)."""
        return self.layers[0].grad_names

    @property
    def dim_names(self) -> Tuple[str, ...]:
        """Get the names of registered dimensions (including unset)."""
        return self.layers[0].dim_names

    @property
    def ref_names(self) -> Tuple[str, ...]:
        """Get the names of registered node references (including unset)."""
        return self.layers[0].ref_names

    def has_dim(self, name: str) -> Optional[bool]:
        """Check whether the model has a dimension of a given name. If the
        dimension is registered but the value is unset, returns None.
        """
        return self.layers[0].has_dim(name)

    def get_dim(self, name: str) -> int:
        """Retrieve the value of a dimension of the given name."""
        return self.layers[0].get_dim(name)

    def set_dim(self, name: str, value: int, *, force: bool = False) -> None:
        """Set a value for a dimension."""
        self.layers[0].set_dim(name, value, force=force)
    ...
    def has_param(self, name: str) -> Optional[bool]:
        ...
        Returns None if the parameter is registered but currently unset.
        """
        return self.layers[0].has_param(name)

    def get_param(self, name: str) -> FloatsXd:
        """Retrieve a weights parameter by name."""
        return self.layers[0].get_param(name)

    def set_param(self, name: str, value: Optional[FloatsXd]) -> None:
        """Set a weights parameter's value."""
        self.layers[0].set_param(name, value)

    def has_grad(self, name: str) -> bool:
        """Check whether the model has a non-zero gradient for a parameter."""
        return self.layers[0].has_grad(name)

    def get_grad(self, name: str) -> FloatsXd:
        """Get a gradient from the model."""
        return self.layers[0].get_grad(name)

    def set_grad(self, name: str, value: FloatsXd) -> None:
        """Set a gradient value for the model."""
        self.layers[0].set_grad(name, value)

    def inc_grad(self, name: str, value: FloatsXd) -> None:
        """Increment the gradient of a parameter by a value."""
        self.layers[0].inc_grad(name, value)

    def has_ref(self, name: str) -> Optional[bool]:
        """Check whether the model has a reference of a given name. If the
        reference is registered but the value is unset, returns None.
        """
        return self.layers[0].has_ref(name)

    def get_ref(self, name: str) -> "BaseModel":
        """Retrieve the value of a reference of the given name."""
        return self.layers[0].get_ref(name)

    def set_ref(self, name: str, value: Optional["BaseModel"]) -> None:
        """Set a value for a reference."""
        self.layers[0].set_ref(name, value)

    def copy(self: "DelegateModel") -> "DelegateModel":
        """
        Create a copy of the model, its attributes, and its parameters. Any child
        layers will also be deep-copied. The copy will receive a distinct `model.id`
        value.
        ...
        copied: DelegateModel[InT, OutT] = DelegateModel(
            self.name,
            self.layers[0].copy(),
            self._func,
            init=self.init,
...
def wrap_model_recursive(
    model: BaseModel, wrapper: Callable[[BaseModel], _ModelT]
) -> _ModelT:
