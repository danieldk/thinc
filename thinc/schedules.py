...
@runtime_checkable
class SchedulerCallable(Protocol):
    def __call__(self, *, key: Tuple[int, str], step: int) -> float:
        ...


@registry.schedules("constant_then.v1")
def constant_then(
    rate: float, steps: int, schedule: SchedulerCallable
) -> SchedulerCallable:
    ...
    def callback(*, key: Tuple[int, str], step: int) -> float:
        if step < steps:
            ...
            return schedule(key=key, step=step)

    return callback


@registry.schedules("constant.v1")
def constant(rate: float) -> SchedulerCallable:
...
def cyclic_triangular(min_lr: float, max_lr: float, period: int) -> SchedulerCallable:
    def callback(*, key: Tuple[int, str], step: int) -> float:
        it = step + 1
